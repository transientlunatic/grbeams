...
for e,epoch in enumerate(epochs):
    ...
    for p,prediction in enumerate(predictions):
        ...
        ax_bns_rate.plot(scenario.bns_rate,np.exp(scenario.bns_rate_pdf), \
                color='k', linestyle=linestyles[p], label=r'%s'%label_str)
        ...
        ax_jet_angle.plot(jetpos.theta,jetpos.jet_pdf_1D, \
                color='k', linestyle=linestyles[p],
                label=r'%s'%label_str)

    ax_bns_rate.set_title('Epoch=%s'%str(epoch))
    ax_bns_rate.legend()
    ...
    ax_jet_angle.set_title('Epoch=%s'%str(epoch))
    ax_jet_angle.legend()
...
pl.subplots_adjust(bottom=0.1,top=0.95)
pl.show()
