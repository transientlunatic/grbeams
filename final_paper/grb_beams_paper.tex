...
scenario, where the rate posterior is relatively uninformative, the results are
dominated by the uncertainty in $\epsilon$: there are now even substantive
...
dramatically different upper limits.

It is interesting to compare these upper limits on the beaming angle with those
in~\cite{2013PhRvL.111r1101C}, where the upper limit on the rate itself is used
as a constraint (rather than transforming the posterior).  This has the
...
with our 95\% confidence upper limit $\theta^{\mathrm{ul}}=1.94^{\circ}$.
