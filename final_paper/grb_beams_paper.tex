...
considering plausible detection scenarios in aLIGO \ish{detailed in [ref]}.  Our ultimate goal is to
...
$\cbcrate$.  The core ingredient to the analysis is the posterior
...
signal.  
\ish{Gravitational wave data analysis pipelines (eg. {\tt FINDCHIRP}~\cite{2012PhRvD..85l2006A})
identify discrete `events' which are characterized by network signal-to-noise ratios, $rho_c$, 
which, for the case of \bns{} searches, indicate the strength of the nspiral \gw{} signal from 
\bns{} coalescence.}
Assume we have a data analysis pipeline (e.g.,
...
known, we are just left with the problem of inferring the signal rate $s$.
...
\end{eqnarray}
%
We see that in this approach, the rate posterior depends only on the number
...
set of observations and $I$ unenumerated prior knowledge.  Our goal is to
...
\end{equation}
%
It is important note that the entire procedure of deriving the jet angle
...
approach is the specification of some prior distribution for $\epsilon$.   \ish{Given 
the lack of information on the value and distribution of $\epsilon$, we choose 
three plausible priors and study their effects on our beaming angle inference. 
