...
\caption{Advanced detector era observing scenarios considered in this
  ...
  resulting in twice the volume being searched per year.
    \dwnote{Why do the numbers in column N not agree with Figure 2?}
  \label{tab:scenarios}}
\end{table}
%
...
\caption{Summary of the \BNS rate posteriors for each of the observing
  ...
  \dwnote{Need to update these numbers to proper intervals.
    A lot of these numbers look suspect when compared with Figure 2.
    How do we get MAP of 0 in the top row...?}
...
\caption{Summary of the beaming angle posteriors from figure
  ...
  angle of $\theta = 10^{\circ}$.
    \dwnote{Need to update these numbers to proper intervals.
    For readability, can the orders in the tables be changed to match the orders
...
computed.
\dwnote{Talk everywhere else about 95\% but here have 0.975?}
