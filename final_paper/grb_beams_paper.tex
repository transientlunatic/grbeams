...
\begin{abstract}
Short gamma-ray bursts are energetic electromagnetic events which are
associated with binary neutron star coalesence events, which are also
promising targets for observation with gravitational wave
detectors. The electromagnetic radiation from a sGRB is believed to be
stongly beamed, whereas the gravitational radiation from the BNS
coalsence is not. We present a method for inferring limits on the
extent of this beaming by comparing the number of sGRBs observed
electromagnetically to the number of BNS events detected in
gravitational waves. We demonstrate that a year of observations with
the 2-detector Advanced LIGO gravitational wave detector is capable of
placing an upper limit of $\sim 80^{\circ}$ on the beaming angle even
in the case of no BNS events being detected, and $\sim 40^{\circ}$
under the assumption that a plausible number are detected
\dwnote{reword this}. We also demonstrate that the same detector
...
\footnote{While we present the entire posterior for only these two
  observing scenarios in this section, we provide an overview of all
  of the observing scenarios in section~\ref{sec:future}.} Since it is
a common assumption in related literature, we also now include a prior
on the \ac{sGRB} efficiency which dictates that all \BNS produce a
...
compute \emph{upper} limit on the 95\% median-centered confidence
interval the beaming angle.
%% \begin{equation}
...
place further constraints on the 95\% confidence interval of the sGRB
