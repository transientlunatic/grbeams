...
\begin{enumerate}
    ...
    \item Draw $N_{\text{\BNS}}$ values of orbital inclination $\iota$ from a distribution which is uniform in $\cos \iota$ in the range $[0,1]$.
...
$N_{\mathrm{GRB}}/N_{\text{\BNS}}$.  Since it is the comparison of
the rates of these events that informs our inference on $\theta$,
studying the ratio $N_{\mathrm{GRB}}/N_{\text{\BNS}}$ provides some
...
distributions on the ratio $N_{\mathrm{GRB}}/N_{\text{\BNS}}$.
...
For example, for the ${\mathcal N}(15,9)$ and ${\mathcal N}(10,13)$ beaming angle populations, the ratios of $N_{\mathrm{GRB}}/N_{\text{\BNS}}$ are almost equal ($\sim 4.8\%$).
...
ST/L000946/1. ARW acknowledges support from the Netherlands Organization for
Scientific Research through the NWO TOP Grant No. 62002444­­-Nissanke.
This document has been assigned the control number
