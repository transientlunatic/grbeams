...
GRB Jet Angles} \color{Black}\\ \\% Title
...
\includegraphics[height=4cm]{uni_glasgow_logo.png} \\ \\% Logo or a photo of you, adjust its dimensions here
\end{minipage}
...
\begin{wrapfigure}{r}{0.5\linewidth}
    \includegraphics[width=\linewidth]{theta_dist_grbfrac.pdf}
...
%\subsection*{Results}
  ...
  \begin{minipage}{\columnwidth}
    ...
    \makeatother
    \centering
    \subfloat[S6/VSR2,3 BNS Rate Posterior]{%
      \includegraphics[width=0.45\linewidth]{S6_rate.pdf}%
      \label{fig:S6ratePosterior}%
    }\qquad%
    \subfloat[S6/VSR2,3 Jet Posterior]{%
      \includegraphics[width=0.45\linewidth]{jet_angle_posterior_iligo.pdf}%
      \label{fig:S6JetPosterior}%
    }
    \caption{\emph{Left}: The binary neutron star merger rate posterior
        constructed from the 90\% rate upper limit (dashed line) reported
        in~\cite{s6lowmass} and the formalism described
    in~\cite{2008CQGra..25j5002B}.  \emph{Right}: the jet angle
posteriors obatined from equation~\ref{eq:transformationmarginalisation}.
Different lines for the jet posterior correspond to the different priors on the
GRB efficiency.} 
  \end{minipage}
...
\begin{center}%\vspace{1cm}
    \includegraphics[width=0.45\linewidth]{aligo_rate_re.pdf}
    \captionof{figure}{Rate posterior PDFs expected following the 2016 and 2022
        observing scenarios described in~\cite{2013arXiv1304.0670L}, assuming
        the `realistic' rate of BNS coalescence from~\cite{2013arXiv1304.0670L}
        (solid vertical line) and using the Bayesian Poisson rate determination
    described in~\cite{Gregory05}.}
\end{center}%\vspace{1cm}

%   \begin{figure}[H]
%       \includegraphics[width=0.45\linewidth]{aligo_rate_re.pdf}
%       \caption{Rate posterior PDFs expected following the 2016 and 2022
%       observing scenarios described in~\cite{2013arXiv1304.0670L}, assuming the
%   so-called `realistic rate of BNS coalescence (solid vertical line) and using the
%   Bayesian Poisson rate determination described in~\cite{Gregory05}.}
%   \end{figure}


\subsection*{\centering Validation: Posteriors For `Known' Jet Angle}
...
\bibliography{grbeams_poster} % Use the example bibliography file sample.bib
}
