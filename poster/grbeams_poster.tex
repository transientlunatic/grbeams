...
\veryHuge \color{NavyBlue} \textbf{Gravitational Wave Constraints On Gamma-Ray
Burst Beaming Angles} \color{Black}\\ \\% Title
...
\subsection*{Results}
  ...
  \begin{minipage}{\columnwidth}
    ...
    \subfloat[Jet Posterior]{%
      \includegraphics[width=0.45\linewidth]{jet_angle_posterior_S6.pdf}%
      \label{fig:}%
    }
    \caption{S6 results}
  \end{minipage}
...
\begin{minipage}{\columnwidth}
  ...
  \caption{Example results for $\theta_{\rm jet}=15^{\circ}$ and binary
  coalescence rates in~\cite{scenarios} to derive a `simulated' GRB rate.}
\end{minipage}

\subsection*{Results: Predictions}
