...
\begin{enumerate}
\item Compute horizon distance from noise curve: $D_{\mathrm{horizon}}$.  For
this, we want a detection threshold.  Let's go with $\rho_c=12$.
\item Find number of galaxies accessible to this noise curve from:
\begin{equation}
N_G = \frac{4}{3} \pi \left( \frac{D_{\textrm{horizon}}}{\textrm{Mpc}}
\right)^3 (2.26)^{-3} (0.0116).
\end{equation}
...
$N_{\mathrm{det}}$
\item Use figure~\ref{fig:far_fig} to get the background rate at the threshold
used for the horizon distance.  The prospects document~\cite{ade_prospects}
conservatively quotes $\rho_c=12$ for FAR$\lesssim 1e-2$\,yr$^{-1}$.
\item Compute \emph{detection} rate posterior.
\item The actual rate posterior is then given by 
\begin{eqnarray}
p(\cbcrate|N_{\textrm{on}},I) & = & p(s|N_{\textrm{on}},I) \left|\frac{\diff
s}{\diff \cbcrate}\right| \\
& = & N_G . p(s|N_{\textrm{on}},I)
...
the only ingredients we \emph{really} need are the detection rates for the
observing scenarios and the the FAR for the threshold used to get those rates!
It's probably a good thing to stick with the threshold/rates in the prospects
document, too.

\begin{figure}
\scalebox{0.6}{\includegraphics{cbc_far_prospectspaper.eps}}
\caption{\label{fig:far_fit} CBC FAR from prospects document~\cite{ade_prospects}}
\end{figure}
